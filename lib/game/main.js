        ...
        var game = ig.Game.extend({
            ...
            _draw_entities: function (type) {
                for (var i = 0, count = this.entities.length; i < count; i++) {
                    ...
                    if (entity instanceof type && !entity._killed) {
                        entity.draw();
                    }
                }
            },
            ...
            _init_foreground_entities: function () {
                ...
                for (var i = 0; i < NUM_FISH; i++) {
                    ...
                    if (jewels.length < NUM_JEWELS) {
                        var jewel = this.spawnEntity(EntityJewel, entity.pos.x, entity.pos.y, 0.5);
            ...
            _schedule_call: function (delay, action, entity) {
                for (var i = 0, count = this.delay_queue.length;
                     i < count && this.delay_queue[i].delay < delay;
                     i++)
                    ;
                var a = arguments;
                var f = function () { action.apply(entity, [].slice.call(a, 3)); };
                this.delay_queue.splice(i, 0, {action: f, delay: delay});
            },
            ...
            on_correct_answer: function (entity) {
                ...
                var jewels_remain = false;

                for (var i = 0, count = this.entities.length; i < count; i++) {
                    var entity = this.entities[i];
                    if (entity instanceof EntityBlowfish && !entity._killed && entity.has_jewel) {
                        jewels_remain = true;
                        break;
                    }
                }
            ...
            on_game_over: function (success) {
                for (var i = 0, count = this.entities.length; i < count; i++) {
                    var entity = this.entities[i];
                    if (!(entity instanceof EntityBackgroundFish) && !entity._killed) {
                        this.removeEntity(entity);
                    }
                }
            ...
            update: function () {
                if (this.delay_queue) {
                    for (var i = 0, count = this.delay_queue.length; i < count; i++) {
                        ...
                        if (item.delay < this.timer.delta()) {
                            item.action();
                        }
                        else {
                            break;
